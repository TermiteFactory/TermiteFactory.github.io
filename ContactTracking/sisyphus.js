	...
	Sisyphus = ( function() {
		...
		function init () {
			...
			return {
							...
							if ( field.is( ":text" ) || field.is( "textarea" ) ) {
								if ( ! self.options.timeout ) {
									if ( $.inArray( this, self.options.occludeFields ) !== -1 ) {
										self.bindSaveDataImmediately( field, prefix, true);
									} else {
										self.bindSaveDataImmediately( field, prefix, false);
									}
						...
						self.findFieldsToProtect( $( this) ).each( function() {
							...
							if ( field.is(":checkbox") ) {
								...
								if ( self.isCKEditorExists() ) {
									...
									if ( $.inArray( this, self.options.occludeFields ) !== -1) {
										self.saveToBrowserStorageOcclude( prefix, value, false);
									} else {
										self.saveToBrowserStorage( prefix, value, false );
									}
				 ...
				 * @return void
				 */
				bindSaveDataImmediately: function( field, prefix, occlude) {
					var self = this;
					if ( 'onpropertychange' in field ) {
						field.get(0).onpropertychange = function() {
							if (occlude) {
								self.saveToBrowserStorageOcclude( prefix, field.val() );
							} else {
				...
				saveToBrowserStorageOcclude: function( key, value, fireCallback ) {
					// Replace value
					var self = this;
